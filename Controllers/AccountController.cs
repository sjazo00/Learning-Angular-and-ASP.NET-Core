            ...
            if(ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    if(result.Succeeded)
                    {
                        ...
                        var token = new JwtSecurityToken(this.config["Tokens:Issuer"], this.config["Tokens:Audience"], claims, signingCredentials: creds, expires: DateTime.UtcNow.AddMinutes(20));
